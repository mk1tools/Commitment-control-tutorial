...
-- pending database transactions (WRKCMTDFN)
select STATE_TIMESTAMP, COMMITMENT_DEFINITION, COMMITMENT_DEFINITION_DESCRIPTION, JOB_NAME, LOGICAL_UNIT_OF_WORK_ID, LOCK_SPACE_ID, LOGICAL_UNIT_OF_WORK_STATE,
       ACTIVATION_GROUP, DEFAULT_LOCK_LEVEL, USER_NAME, 
       LOCAL_CHANGES_PENDING, LOCAL_JOURNAL_CHANGES_PENDING, LOCAL_RECORD_CHANGES_PENDING, LOCAL_OBJECT_CHANGES_PENDING, 
       JOB_ACTIVE, LOCK_SCOPE, NUMBER_SAVEPOINTS, COMMIT_ROLLBACK_END, EXPLICIT_COMMIT_ROLLBACK, 
       SQL_DYNAMIC_COMMIT, SQL_DYNAMIC_ROLLBACK, SQL_HOLD_COMMIT, COMMIT_DURABLE, 
       NUMBER_COMMITS, NUMBER_ROLLBACKS
  from DB_TRANSACTION_INFO
  where USER_NAME = :Utente
  order by STATE_TIMESTAMP desc;

-- informazioni sullo stato transazione a livello di record
select JOB_NAME, LIBRARY_NAME, FILE_NAME, MEMBER_NAME, FILE_CHANGES_COMMITTED, FILE_CHANGES_ROLLED_BACK, FILE_CHANGES_PENDING, 
       LOCK_LEVEL, STATUS, CONCURRENT_ACCESS_RESOLUTION, 
       JOURNAL_LIBRARY, JOURNAL_NAME, COMMIT_CYCLE
  from DB_TRANSACTION_INFO,
    lateral (select *
               from table(DB_TRANSACTION_RECORD_INFO(LOCK_SPACE_ID))) as DBT
  where USER_NAME = :Utente               
  order by STATE_TIMESTAMP desc, LIBRARY_NAME, FILE_NAME;
    
-- informazioni sullo stato transazione a livello di giornale
select JOURNAL_LIBRARY, JOURNAL_NAME, COMMIT_CYCLE, RECORD_LOCKS, PENDING_CHANGES
  from DB_TRANSACTION_INFO,
    lateral (select *
               from table(DB_TRANSACTION_JOURNAL_INFO(LOCK_SPACE_ID))) as JRN
  where USER_NAME = :Utente               
  order by STATE_TIMESTAMP desc;
